import { X, LayoutDashboard, MapPin, Truck, ChefHat, Leaf, Navigation, ClipboardList } from "lucide-react";
import { motion } from "framer-motion";

const menuItems = [
  { name: "Dashboard",        icon: LayoutDashboard, action: "dashboard",      description: "Overview & stats"       },
  { name: "Optimized Routes", icon: Navigation,      action: "selectRoute",    description: "AI-powered route plan"  },
  { name: "Active Delivery",  icon: Truck,           action: "activeDelivery", description: "Current delivery"       },
  { name: "Past Deliveries",  icon: ClipboardList,   action: "pastDeliveries", description: "Completed history"      },
];
      ...
      <div className="flex items-center justify-between px-5 py-5 border-b border-[rgba(0,0,0,0.06)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-[var(--muted-text)] mt-0.5 font-medium">Robin Portal</p>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="px-3 py-4 border-t border-[rgba(0,0,0,0.06)]">
        <div className="rounded-2xl bg-gradient-to-br from-[var(--green-primary)]/10 to-[var(--green-primary)]/5 border border-[var(--green-primary)]/15 px-4 py-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-bold text-[var(--green-primary)]">Night Robin</p>
          </div>
          <p className="text-[11px] text-[var(--muted-text)] leading-relaxed">
            Every delivery you make feeds someone in need tonight.
          </p>
        </div>
      </div>
    </motion.aside>
