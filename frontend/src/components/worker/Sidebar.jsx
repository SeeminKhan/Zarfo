import { X, Home, ClipboardList, ChefHat, Leaf } from "lucide-react";
import { motion } from "framer-motion";

const menuItems = [
  { name: "Browse Food",  icon: Home,          action: "feed",        description: "Free donation meals" },
  { name: "My Requests",  icon: ClipboardList, action: "myRequests",  description: "Track your requests"  },
];

export function Sidebar({ open, onClose, setActivePage, activePage }) {
  const handleClick = (action) => { setActivePage(action); onClose(); };
      ...
      <div className="flex items-center justify-between px-5 py-5 border-b border-[rgba(0,0,0,0.06)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-[var(--muted-text)] mt-0.5 font-medium">Worker Portal</p>
          </div>
        </div>
        <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center text-[var(--muted-text)] hover:bg-[var(--bg-color-light)] transition-all">
          <X size={15} />
        </button>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="text-left min-w-0">
                ...
                <p className={`text-[10px] mt-0.5 leading-none ${isActive ? "text-white/70" : "text-[var(--muted-text)]"}`}>{item.description}</p>
              </div>
            </button>
          );
        })}
      ...
      <div className="px-3 py-4 border-t border-[rgba(0,0,0,0.06)]">
        <div className="rounded-2xl bg-gradient-to-br from-[var(--green-primary)]/10 to-[var(--green-primary)]/5 border border-[var(--green-primary)]/15 px-4 py-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-bold text-[var(--green-primary)]">Free Meals</p>
          </div>
          <p className="text-[11px] text-[var(--muted-text)] leading-relaxed">Surplus food donated by hotels, delivered by Night Robins.</p>
        </div>
      </div>
    </motion.aside>
  );
